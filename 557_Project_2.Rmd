...
fit.glm <- glm(class~., seismic.train, family=binomial(link="logit"))

glm.link.scores <- predict(fit.glm, seismic.test, type="link")

glm.response.scores <- predict(fit.glm, seismic.test, type="response")

score_data <- data.frame(link=glm.link.scores, 
                         response=glm.response.scores,
                         class=seismic.test$class,
                         stringsAsFactors=FALSE)


#score_data %>% 
#  ggplot(aes(x=link, y=response, col=class)) + 
#  scale_color_manual(values=c("black", "red")) + 
#  geom_point() + 
#  geom_rug() + 
#  ggtitle("Both link and response scores put cases in the same order")

#plot.roc(roc(seismic.test$class, glm.response.scores, direction="<", auc=TRUE),
#     col="blue", pch=18,lwd=3, main="ROC Curve", print.thres=TRUE, print.auc=TRUE)

tt <- roc(seismic.test$class, glm.response.scores, direction="<")

plot.roc(tt, col="blue", auc.polygon=TRUE,main="ROC Curve",print.auc=TRUE)
...
#glm_simple_roc <- simple_roc(seismic.test$class=="TRUE", glm.link.scores)
